...
@section('content')
    <div class="container mx-auto mt-8 px-4">
        <div class="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
            ...
            <form action="{{ route('guide.update', ['guide' => $guide->id]) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <input type="text" id="nama_guide" name="nama_guide"
                        value="{{ old('nama_guide', $guide->nama_guide) }}" required class="w-full border rounded p-2">
                    @error('nama_guide')
                ...
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2" for="salary">Gaji</label>
                    <input type="text" name="salary" id="salary" value="{{ old('salary', $guide->salary) }}"
                ...
                <div>
                    ...
                    <input type="text" id="nomer_hp" name="nomer_hp" value="{{ old('nomer_hp', $guide->nomer_hp) }}"
                ...
                <div>
                    ...
                    <input type="text" id="alamat" name="alamat" value="{{ old('alamat', $guide->alamat) }}" required
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $guide->email) }}" required
                        class="w-full border rounded p-2">
                    @error('email')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="bahasa" name="bahasa" value="{{ old('bahasa', $guide->bahasa) }}"
                        class="w-full border rounded p-2">
                    @error('bahasa')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    @if (!empty($guide->foto))
                        <div class="mt-2">
                            ...
                            <img src="{{ asset('storage/' . $guide->foto) }}" alt="Foto Guide"
                ...
                <div>
                    ...
                    <select id="status" name="status" required class="w-full border rounded p-2">
                        ...
                        <option value="2" {{ old('status', $guide->status) == 2 ? 'selected' : '' }}>Sedang Guiding
                        </option>
                        <option value="3" {{ old('status', $guide->status) == 3 ? 'selected' : '' }}>Tidak Aktif
                        </option>
                    </select>
                    @error('status')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <div class="flex justify-between">
                    <a href="{{ route('guide.index') }}"
                        class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded">Kembali</a>
                    <button type="submit"
                        class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">Simpan
                        Perubahan</button>
                </div>
            </form>
