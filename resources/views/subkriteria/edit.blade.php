...
<div class="container mx-auto mt-8 px-4">
    <div class="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
        <h1 class="text-3xl font-bold mb-4 text-gray-800">Edit Subkriteria</h1>
        ...
        <form action="{{ route('subkriteria.update', $subkriteria->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi" id="deskripsi" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ $subkriteria->deskripsi }}</textarea>
            </div>

            <div class="mb-4">
                <label for="is_core_factor" class="block text-gray-700 text-sm font-bold mb-2">Core Factor:</label>
                <input type="checkbox" name="is_core_factor" id="is_core_factor" value="1" class="mr-2 leading-tight" {{ $subkriteria->is_core_factor ? 'checked' : '' }}>
            </div>

            <div class="mb-4 ">
                <label for="profil_standar" class="block text-gray-700 text-sm font-bold mb-2">Profil Standar:</label>
                <input type="number" min="1" max="5" name="profil_standar" id="profil_standar" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{$subkriteria->profil_standar}}">
            </div>
