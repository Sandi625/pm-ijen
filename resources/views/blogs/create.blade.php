            ...
            <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug') }}"
                        class="mt-1 block w-full px-4 py-2 border rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                        placeholder="Slug otomatis" readonly required>
                    @error('slug')
                        <div class="text-red-500 text-sm mt-2">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    <label for="image" class="block text-lg font-medium text-gray-700">Gambar
                ...
                <div class="mb-4">
                    ...
                    <input type="checkbox" id="status" name="status" value="1" class="mt-1"
                        {{ old('status') ? 'checked' : '' }}>
                    @error('status')
                        <div class="text-red-500 text-sm mt-2">{{ $message }}</div>
                    @enderror
